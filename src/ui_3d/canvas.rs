...
trait Renderable {
    ...
    unsafe fn paint(&self, gl: &Context, mvp: cgmath::Matrix4<f32>, cam_pos: cgmath::Point3<f32>);
    unsafe fn fill_vertex_buffer(&self, gl: &Context, mvp: cgmath::Matrix4<f32>, cam_pos: cgmath::Point3<f32>) -> usize;
}
...
impl Renderable for InstancedRenderObject {
    ...
    unsafe fn paint(&self, gl: &Context, mvp: cgmath::Matrix4<f32>, cam_pos: cgmath::Point3<f32>) {
        let count = self.fill_vertex_buffer(gl, mvp, cam_pos);
    ...
    unsafe fn fill_vertex_buffer(&self, gl: &Context, mvp: cgmath::Matrix4<f32>, cam_pos: cgmath::Point3<f32>) -> usize {
        ...
        let light_pos: [f32; 3] = [0.0, 2.0, 0.0];
...
impl Renderable for RenderObject {
    ...
    unsafe fn paint(&self, gl: &Context, mvp: cgmath::Matrix4<f32>, cam_pos: cgmath::Point3<f32>) {    
        self.fill_vertex_buffer(gl, mvp, cam_pos);
    ...
    unsafe fn fill_vertex_buffer(&self, gl: &Context, mvp: cgmath::Matrix4<f32>, _cam_pos: cgmath::Point3<f32>) -> usize {
        gl.use_program(Some(self.program));
        gl.bind_vertex_array(Some(self.vao));
...
impl Canvas {
    pub fn new(gl: &Context) -> Option<Self> {
        unsafe {
            ...
            let (domino_obj, light_obj) = init_vertex_buffer(&gl);
    ...
    pub fn paint(&self, gl: &Context, mvp: cgmath::Matrix4<f32>, cam_pos: cgmath::Point3<f32>) {
        unsafe {
            ...
            self.domino_obj.paint(gl, mvp, cam_pos);
            self.light_obj.paint(gl, mvp, cam_pos);
        }
    }
}
...
unsafe fn init_vertex_buffer(gl: &Context) -> (InstancedRenderObject, RenderObject) {
